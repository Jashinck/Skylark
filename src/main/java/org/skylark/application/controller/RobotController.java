...
public class RobotController {
     ...
     * 创建新的阿里云 ARTC 会话
     * 
     * <p>Creates an AliRTC channel, generates authentication info, and returns
     * connection info for the client to connect via AliRTC Web SDK.</p>
     * 
     * @param request Create session request with user ID
     * @return AliRTC connection response with app ID, channel ID, and auth info
     */
    @PostMapping("/alirtc/session")
    public ResponseEntity<AliRTCConnectionResponse> createAliRTCSession(
            @RequestBody CreateSessionRequest request) {
        try {
            logger.info("Creating AliRTC session for user: {}", request.getUserId());
            ...
            try {
                ...
                authInfo = node.has("authInfo") ? node.get("authInfo").toString() : null;
            } catch (Exception parseError) {
            ...
            AliRTCConnectionResponse response = new AliRTCConnectionResponse(
                ...
                "created", "AliRTC session created successfully"
            );
            
            logger.info("AliRTC session created: {}", sessionId);
            return ResponseEntity.ok(response);
            
        } catch (Exception e) {
            logger.error("Failed to create AliRTC session", e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
    ...
    public ResponseEntity<Void> closeAliRTCSession(@PathVariable String sessionId) {
        try {
            logger.info("Closing AliRTC session: {}", sessionId);
            ...
            logger.error("Failed to close AliRTC session: {}", sessionId, e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
        }
    }
}
