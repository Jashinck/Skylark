 ...
 * the Skylark voice assistant service. It handles configuration loading and
...
public class SkylarkApplication {
    ...
    /**
     * Main entry point for the Skylark Voice Assistant application.
     * 
     ...
     * java -jar skylark.jar config/config.yaml                     # Use config from classpath
    ...
    public static void main(String[] args) {
        try {
            logger.info("========================================");
            logger.info("  Skylark Voice Assistant Starting");
            logger.info("  云雀智能语音助手启动中");
            logger.info("========================================");
            ...
            ConfigurableApplicationContext context = SpringApplication.run(SkylarkApplication.class, args);
            ...
            logger.info("========================================");
            logger.info("  Skylark Voice Assistant Started");
            logger.info("  云雀智能语音助手已启动");
            logger.info("========================================");
            
        } catch (Exception e) {
            logger.error("Failed to start Skylark Voice Assistant", e);
            System.exit(1);
        }
    }
    ...
    private static void registerShutdownHook(ConfigurableApplicationContext context) {
        Runtime.getRuntime().addShutdownHook(new Thread(() -> {
            logger.info("Shutdown signal received, closing Skylark Voice Assistant...");
            try {
                context.close();
                logger.info("Skylark Voice Assistant shut down successfully");
            } catch (Exception e) {
