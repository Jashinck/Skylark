...
│  │  - WebRTC Adapters (Kurento/LiveKit/Agora)          │     │
│  │  - WebRTC Strategy (WebSocket/Kurento/LiveKit/Agora)│    │
│  └────────────────────────────────────────────────────┘     │
│  ┌────────────────────────────────────────────────────┐     │
│  │  配置 (Configuration)                               │     │
...
└──────────────┬────────────────┬─────────────────┬───────────┘
               ...
               ↓                ↓                 ↓
      ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐
      │    Vosk      │  │   MaryTTS    │  │ Silero VAD   │  │ AgentScope   │
      │   (ASR)      │  │   (TTS)      │  │   (VAD)      │  │  (Agent)     │
      └──────────────┘  └──────────────┘  └──────────────┘  └──────┬───────┘
               │                │                                   │
               ↓                ↓                                   ↓
      ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐
      │   Kurento    │  │   LiveKit    │  │    Agora     │  │  DeepSeek    │
      │  Media Server│  │   Server     │  │  PAAS RTC   │  │   LLM API    │
      └──────────────┘  └──────────────┘  └──────────────┘  └──────────────┘
```
...
**组件**:
- `service/`: 应用服务
  - `AgentService`: AI Agent 服务（AgentScope 框架，ReAct 推理、工具调用、记忆管理）
  - `ASRService`: 自动语音识别服务（Vosk）
  - `TTSService`: 文本转语音服务（MaryTTS）
  - `VADService`: 语音活动检测服务（Silero VAD）
  - `WebRTCService`: WebRTC 实时通信服务
  - `OrchestrationService`: 编排服务（协调 VAD → ASR → AgentService → TTS 流程）
- `dto/`: 数据传输对象

**AgentService 详解**:
- **框架**: AgentScope 1.0.9
- **核心组件**:
  - `ReActAgent` - ReAct 推理引擎（自动多步推理）
  - `OpenAIChatModel` - LLM 模型（支持 DeepSeek、GPT-4o、千问等）
  - `Toolkit` - 工具注册与调用（@Tool 注解）
  - `InMemoryMemory` - Per-Session 对话记忆
...
- `adapter/`: 适配器实现
  ...
  - `webrtc/`: WebRTC 适配器
    ...
    - `AgoraClientAdapter`: 声网 Agora PAAS RTC 适配器
...
WebRTCChannelStrategy (接口)
  ...
  └── AgoraChannelStrategy (声网 Agora PAAS RTC 方案)
```
...
- **WebRTC**: Kurento Client 6.18.0 / LiveKit Server SDK 0.12.0 / Agora Linux SDK 4.4.31.4
- **构建**: Maven
...
**更新时间**: 2026-02-15  
**架构版本**: 1.2.0 (DDD分层架构 + WebRTC 策略模式 + PAAS RTC 集成)
